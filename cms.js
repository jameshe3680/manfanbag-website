(async()=>{
 ...
 function fillProduct(el,row){
 const oldImage=el.querySelector('.product-image');
 const number=el.querySelector('.number')?.textContent||`${String(document.querySelectorAll('.collection-item').length+1).padStart(2,'0')} / COLLECTION`;
 const parts=(row.data.body||'').split(/\n\s*\n/).filter(Boolean);
 const structured=Object.hasOwn(row.data,'lead');
 const lead=structured?row.data.lead:(parts.shift()||'');
 const body=structured?parts:parts.slice(0,1);
 const features=structured?(row.data.features||'').split('\n').filter(x=>x.trim()):parts.slice(1);
 el.className='collection-item';el.style.removeProperty('display');el.style.removeProperty('padding');el.style.removeProperty('border-bottom');el.replaceChildren();
 if(row.data.image){const img=document.createElement('img');img.src=row.data.image;img.alt=row.data.title;img.loading='lazy';img.style='width:100%;aspect-ratio:1;object-fit:cover';el.append(img);}else if(oldImage)el.append(oldImage);
 const details=document.createElement('div');details.className='collection-details';el.append(details);
 const add=(tag,text,cls)=>{const node=document.createElement(tag);node.textContent=text;if(cls)node.className=cls;details.append(node);return node;};
 add('span',number,'number');add('h2',row.data.title);if(lead)add('p',lead,'lead');if(row.data.summary)add('p',row.data.summary);
 body.forEach(t=>add('p',t));
 if(features.length){const ul=add('ul','');features.forEach(t=>{const li=document.createElement('li');li.textContent=t;ul.append(li);});}
 const a=add('a',`Discuss ${row.data.title} →`,'btn btn-fill');a.href=new URL('start-project/index.html',base);
 }
 function fill(el,row){if(row.kind==='product'){fillProduct(el,row);return;}const originalImage=el.querySelector(".product-image");el.style.display="block";el.replaceChildren();if(originalImage&&!row.data.image)el.append(originalImage);if(row.data.image){const img=document.createElement('img');img.src=row.data.image;img.alt=row.data.title;img.loading='lazy';img.style='width:100%;max-height:340px;object-fit:cover';el.append(img);}const h=document.createElement('h2');h.textContent=row.data.title;el.append(h);if(row.data.summary){const summary=document.createElement('p');summary.textContent=row.data.summary;el.append(summary);}for(const text of (row.data.body||'').split('\n\n')){const p=document.createElement('p');p.textContent=text;p.style.whiteSpace='pre-wrap';el.append(p);}if(row.kind==='product'){const a=document.createElement('a');a.href=new URL('start-project/index.html',base);a.className='btn btn-fill';a.textContent='Discuss this collection →';el.append(a);}}
